...
import { AddArticleSchema } from '../../_components/editor/inactive-article-table/article-schema';
...
export default function EditorEditedUpdateArticle() {
  ...
  const {
    ...
    defaultValues: {
      categoryId: gState.updateArticle?.categoryId,
      title: gState.updateArticle?.title,
      description: gState.updateArticle?.description,
      content: gState.updateArticle?.content,
      status: `${gState.updateArticle?.status}`,
    },
  });
  ...
  const onSubmit = async (v) => {
    ...
    loading.set(true);
    await editorFn.updateEditedArticle({
      ...v,
      status: v.status == 'true' ? true : false,
      id: gState.updateArticle?.id,
      date: BigInt(Date.now()) * BigInt(1000000),
    });
    loading.set(false);
  };
  ...
  useEffect(() => {
    editorFn.getData(TabEnum.CATEGORIES);
  }, []);

  return (
    <form onSubmit={handleSubmit(onSubmit)} className="mt-8 font-poppins">
      <h1 className="text-3xl font-bold">{`Update ${gState.updateArticle?.title}`}</h1>
      <p className="text-gray-500 pt-1">Edit the form to update the article</p>
            ...
            <Controller
              ...
              render={({ field: { onChange, onBlur, value, ref } }) => (
                <Select onValueChange={onChange} defaultValue={value}>
                  ...
                  <SelectContent>
                    <SelectGroup>
                      {editorState.contents.categories.map((category) => (
        ...
        <div className="flex flex-col">
          ...
          <div className="flex-1 bg-white rounded-lg p-6 mt-2">
            <div className="text-center font-black uppercase text-sm tracking-widest text-icp-yellow">
              {editorState.contents.categories.find((c) => c.id === watchCat)?.name}
            </div>
